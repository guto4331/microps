...
static void arp_dump(const uint8_t* data, size_t len) {
    ...
    fprintf(stderr, "        sha: %s\n",
            ether_addr_ntop(msg->sha, addr, sizeof(addr)));
    memcpy(&spa, msg->spa, sizeof(spa));
    fprintf(stderr, "        spa: %s\n", ip_addr_ntop(spa, addr, sizeof(addr)));
    fprintf(stderr, "        tha: %s\n",
            ether_addr_ntop(msg->tha, addr, sizeof(addr)));
...
static void arp_cache_delete(struct arp_cache* cache) {
    ...
    debugf("delete ARP cache: pa=%s, ha=%s\n",
...
static struct arp_cache* arp_cache_update(ip_addr_t pa, const uint8_t* ha) {
    ...
    debugf("update ARP cache: pa=%s, ha=%s\n",
...
static struct arp_cache* arp_cache_insert(ip_addr_t pa, const uint8_t* ha) {
    ...
    cache->pa = pa;
    gettimeofday(&cache->timestamp, NULL);
    debugf("insert ARP cache: pa=%s, ha=%s\n",
           ip_addr_ntop(pa, addr1, sizeof(addr1)),
           ether_addr_ntop(ha, addr2, sizeof(addr2)));
    return cache;
}
...
static int arp_reply(struct net_iface* iface, const uint8_t* tha, ip_addr_t tpa,
    ...
    debugf("dev=%s, len=%zu\n", iface->dev->name, sizeof(reply));
    arp_dump((const uint8_t*)&reply, sizeof(reply));
    return net_device_output(iface->dev, ETHER_TYPE_ARP, (const uint8_t*)&reply,
                             sizeof(reply), dst);
}
...
static void arp_input(const uint8_t* data, size_t len, struct net_device* dev) {
    ...
    int merge = 0;  // 更新の可否flag

    if (len < sizeof(*msg)) {
        errorf("ARP: too short packet (len=%zu)\n", len);
        return;
    }
    msg = (struct arp_ether_ip*)data;
    if (ntoh16(msg->hdr.hrd) != ARP_HRD_ETHER ||
        msg->hdr.hln != ETHER_ADDR_LEN) {
        errorf("ARP: unsupported hardware format\n");
        return;
    }
    if (ntoh16(msg->hdr.pro) != ARP_PRO_IP || msg->hdr.pln != IP_ADDR_LEN) {
        errorf("ARP: unsupported protocol format\n");
        return;
    }
...
int arp_resolve(struct net_iface* iface, ip_addr_t pa, uint8_t* ha) {
    ...
    if (iface->dev->type != NET_DEVICE_TYPE_ETHERNET) {
        errorf("ARP: unsupported device type %u\n", iface->dev->type);
        return ARP_RESOLVE_ERROR;
    }
    if (iface->family != NET_IFACE_FAMILY_IP) {
        errorf("ARP: unsupported protocol address %u\n", iface->family);
        return ARP_RESOLVE_ERROR;
    }
    mutex_lock(&mutex);
    cache = arp_cache_select(pa);
...
static void arp_timer_handler(void) {
    ...
    for (entry = caches; entry < tailof(caches); entry++) {
        switch (entry->state) {
            case ARP_CACHE_STATE_FREE:
            case ARP_CACHE_STATE_STATIC:
                break;
            default:
...
int arp_init(void) {
    ...
    if (net_timer_register(interval, arp_timer_handler) < 0) {
        errorf("ARP: net_timer_register() failed");
        return -1;
    }
    return 0;
}
