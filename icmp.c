...
struct icmp_hdr {
    ...
    uint8_t code;
    uint16_t sum;
    uint32_t values;  // ICMPメッセージの種類によって扱いが異なる
};

/* echo / echo reply はこれにキャストする */
struct icmp_echo {
    uint8_t type;
    uint8_t code;
    uint16_t sum;
...
void icmp_input(const uint8_t* data, size_t len, ip_addr_t src, ip_addr_t dst,
    ...
    if (len < ICMP_HDR_SIZE) {
        errorf("ICMP packet is too short: %zu bytes", len);
        return;
    }
    hdr = (struct icmp_hdr*)data;
    ...
    debugf("ICMP packet received: src=%s, dst=%s, len=%zu",
...
int icmp_output(uint8_t type, uint8_t code, uint32_t values,
    ...
    debugf("ICMP packet send: src=%s, dst=%s, len=%zu",
           ip_addr_ntop(src, addr1, sizeof(addr1)),
