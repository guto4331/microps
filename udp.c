...
/*
    擬似ヘッダ：インターネット層の情報も含めたヘッダ（チェックサムの計算に使う）
*/
struct pseudo_hdr {
    uint32_t src;
    uint32_t dst;
...
struct udp_queue_entry {
    struct ip_endpoint foreign;  // 送信元
    uint16_t len;
    uint8_t data[];
};
...
static void udp_input(const uint8_t* data, size_t len, ip_addr_t src,
    ...
    if (len < sizeof(*hdr)) {
        errorf("UDP packet too short: len=%u", len);
        return;
    }
    hdr = (struct udp_hdr*)data;
    if (len != ntoh16(hdr->len)) {
        errorf("UDP length mismatch: len=%u, hdr->len=%u", len,
               ntoh16(hdr->len));
        return;
    }
    ...
    if (cksum16((uint16_t*)data, len, psum) != 0) {
        errorf("UDP checksum mismatch: src=%s, dst=%s",
               ip_addr_ntop(src, addr1, sizeof(addr1)),
               ip_addr_ntop(dst, addr2, sizeof(addr2)));
        return;
    }
    debugf("UDP packet: src=%s:%u, dst=%s:%u, len=%u, payload=%u",
    ...
    debugf("pushed to queue: id=%d, num=%d", udp_pcb_id(pcb), pcb->queue.num);
    // 休止中のタスクにエントリの追加を通知
    sched_wakeup(&pcb->ctx);
    mutex_unlock(&mutex);
}
...
ssize_t udp_output(struct ip_endpoint* src, struct ip_endpoint* dst,
    ...
    if (len > IP_PAYLOAD_SIZE_MAX - sizeof(*hdr)) {
        errorf("UDP packet is too large: len=%zu", len);
        return -1;
    }
    ...
    debugf("UDP output: src=%s, dst=%s, len=%u, payload=%zu",
...
int udp_init(void) {
    ...
    if (net_event_subscribe(event_handler, NULL) < 0) {
...
int udp_open(void) {
    ...
    id = udp_pcb_id(pcb);
    mutex_unlock(&mutex);
    debugf("UDP socket opened: id=%d", id);
    return id;
}
...
int udp_close(int id) {
    ...
    pcb = udp_pcb_get(id);
    if (!pcb) {
        errorf("invalid UDP socket id: id=%d", id);
        mutex_unlock(&mutex);
        return -1;
    }
    udp_pcb_release(pcb);
    mutex_unlock(&mutex);
    debugf("UDP socket closed: id=%d", id);
    return 0;
}
...
int udp_bind(int id, struct ip_endpoint* local) {
    ...
    if (exist && exist != pcb) {
        errorf("UDP port already in use: %s",
    ...
    debugf("UDP socket bound: id=%d, local=%s", id,
...
ssize_t udp_sendto(int id, uint8_t* data, size_t len,
    ...
    if (local.addr == IP_ADDR_ANY) {
        /* 送信元を自動選択 */
        iface = ip_route_get_iface(foreign->addr);  // IPルーティング
        if (!iface) {
            errorf("could not reach any iface: addr=%s",
                   ip_addr_ntop(foreign->addr, addr, sizeof(addr)));
            mutex_unlock(&mutex);
            return -1;
        }
        local.addr = iface->unicast;
        debugf("local address set automatically: addr=%s",
    ...
    if (!pcb->local.port) {
        ...
        for (p = UDP_SOURCE_PORT_MIN; p <= UDP_SOURCE_PORT_MAX; p++) {
            if (!udp_pcb_select(local.addr, hton16(p))) {
                ...
                debugf("local port set automatically: port=%u", p);
                break;
            }
        }
        if (!pcb->local.port) {
            errorf("no available UDP source port");
            mutex_unlock(&mutex);
            return -1;
        }
    }
